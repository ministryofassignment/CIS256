...
function Add-Disk ([int] $Number, [string] $VMName)
{
    ...
    $vhdpath = (Get-VM -Name $vmname).Path + "\Virtual Hard Disks\"
...
$vmnames = "CIS256FR-DC1","CIS256FR-DC2","CIS256FR-RTR"

foreach ($vmname in $vmnames)
{
    Create-VM -VMName $vmname -RAM 2GB -OS SVR2019G -Clone -switch WAN
}

$vmnames = "CIS256FR-Client"
foreach ($vmname in $vmnames)
{
    Create-VM -VMName $vmname -RAM 2GB -OS W10 -Clone -switch WAN
}

Get-VM | Set-VM -DynamicMemory 

Get-VMNetworkAdapter -VMName CIS256FR-DC1 | Remove-VMNetworkAdapter
Add-NetworkAdapter -VMName CIS256FR-DC1 -Switch LAN2

Get-VMNetworkAdapter -VMName CIS256FR-DC2 | Remove-VMNetworkAdapter
Add-NetworkAdapter -VMName CIS256FR-DC2 -Switch LAN3

Add-NetworkAdapter -VMName CIS256FR-RTR -Switch LAN2
Add-NetworkAdapter -VMName CIS256FR-RTR -Switch LAN3

Get-VMNetworkAdapter -VMName CIS256FR-Client | Remove-VMNetworkAdapter
Add-NetworkAdapter -VMName CIS256FR-Client -Switch LAN2
